...
### Evaluations 
#### **Google Pixel (Android with Tensor SoC and Titan M- bzw. M2-Chip)** 
- **Accessibility and Usability:**  
...
The keystore *Knox Vault Storage* can store an arbitrary number of keys. The Knox Vault Processor solely executes algorithms preloaded by Samsung. We assume that the evaluated devices are equipped with Samsung's own TEE implementation *TEEGRIS*, which was introduced with the Samsung Galaxy S10. *TEEGRIS* can be used to secure the KeyStore in hardware, if StrongBox cannot be used.  
